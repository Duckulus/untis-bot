  ...
  cron.schedule('0 7 * * Monday-Friday', async () => {
    ...
    for (let user of users) {
      ...
      if (!untis_school || !untis_username || !untis_password || !untis_eap || !user) {
        await client.sendMessage(contact._serialized, `You are subscribed but there are no credentials associated with your account. Use ${COMMAND_PREFIX}unsubscribe to unsubscribe or ${COMMAND_PREFIX}untis to enter your credentials.`)
        continue
      }
      ...
      if (timetable.filter(l =>
        l.su[0].longname.toLowerCase().includes("sport") && l.substText?.toLowerCase() != "eva"
      ).length) {
        msg += "\n\n"
